...
function warnIfLow(info: GroqRateLimitInfo): void {
  const { remainingRequestsDay, limitRequestsDay, remainingTokensMinute, limitTokensMinute } = info;

  if (remainingRequestsDay !== null && limitRequestsDay !== null && limitRequestsDay > 0) {
    const pct = remainingRequestsDay / limitRequestsDay;
    if (remainingRequestsDay === 0) {
      console.error('[groq] ⛔ Daily request limit reached. Quota resets in 24h.');
    } else if (pct < 0.05) {
      console.warn(`[groq] 🔴 Daily requests critically low: ${remainingRequestsDay} / ${limitRequestsDay} (${Math.round(pct * 100)}%)`);
    } else if (pct < 0.20) {
      console.warn(`[groq] 🟡 Daily requests running low: ${remainingRequestsDay} / ${limitRequestsDay} (${Math.round(pct * 100)}%)`);
    }
  }

  if (remainingTokensMinute !== null && limitTokensMinute !== null && limitTokensMinute > 0) {
    const pct = remainingTokensMinute / limitTokensMinute;
    if (remainingTokensMinute === 0) {
      console.warn('[groq] 🟠 Per-minute token limit reached. Next request may be rate limited (429).');
    } else if (pct < 0.20) {
      console.warn(`[groq] 🟡 Per-minute tokens low: ${remainingTokensMinute} / ${limitTokensMinute} TPM (${Math.round(pct * 100)}%)`);
    }
  }
}
...
async function generateWithGroq(options: GenerateOptions): Promise<GenerateResult> {
  ...
  if (!apiKey) {
    throw new Error('GROQ_API_KEY is not set. Get a free key at console.groq.com and add to .env.local');
  }
  ...
  if (!response.ok) {
    if (response.status === 429) {
      ...
      throw new Error(`Groq rate limit exceeded (429).${retryAfter ? ` Retry after ${retryAfter}s.` : ''} Check quota at console.groq.com.`);
    }
  ...
  if (!text) {
    console.error('[groq] Empty response. Full API response:', JSON.stringify(data, null, 2));
